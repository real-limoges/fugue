defmodule FugueWeb.MoodLive.GapAnalysis do
  @moduledoc "Renders the breath-timeline visualization of silences."

  use FugueWeb, :live_component

  alias FugueWeb.MoodLive.GapBreathTimeline

  def render(assigns) do
    ~H"""
    <div class="bg-base-200 rounded-lg p-4">
      <div class="flex items-baseline justify-between mb-1">
        <h3 class="text-sm font-semibold text-base-content/90">Breath timeline</h3>
        <span class="text-[10px] uppercase tracking-widest text-base-content/45">
          width = duration · height = certainty · color = imputed state
        </span>
      </div>
      <p class="text-xs text-base-content/45 mb-3 leading-snug">
        Each shape is a stretch I went quiet. Taller means the model was more sure what state I was in while the page was dark.
      </p>

      <GapBreathTimeline.timeline
        transitions={@gap_transitions}
        imputed_memberships={@imputed_memberships}
        date_range={@date_range}
        cluster_colors={@cluster_colors}
        cluster_names={@cluster_names}
      />
    </div>
    """
  end
end
